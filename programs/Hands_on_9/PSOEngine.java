...
public class PSOEngine {
    ...
    public void initParticles(Particle[] particles) {
        ...
        for (int i=0; i<particles.length;i++) {
            ...
            for (int j=0; j<numDimensions; j++) {
                positions[j] = rand.nextDouble()*100;
                velocities[j] = 0;
            }
            //Create the particle
    ...
    /**
     * Method to calculate the fitness of a particle using the determinant coefficient
    ...
    public double evaluateFitness(double[] positions) {
      double r2;
       // first pass
       double slope  = positions[1];
       double intercept =positions[0];
